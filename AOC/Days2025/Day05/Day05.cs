...
public class Day05 : Day<Inventory, int, HashSet<(ulong Lower, ulong Upper)>, ulong>
{
    ...
    protected override ulong SolvePart2(HashSet<(ulong Lower, ulong Upper)> ranges) => ranges
        .Aggregate((ulong)0, (current, range) => current + (range.Upper - range.Lower) + 1);
    
    private static HashSet<(ulong Lower, ulong Upper)> AddNewRange(HashSet<(ulong Lower, ulong Upper)> inventoryRanges, (ulong, ulong) newRange)
    {
        var overlappingRanges = new HashSet<(ulong Lower, ulong Upper)>();
        foreach (var range in inventoryRanges.ToList().Where(range => Overlap(range, newRange)))
        {
    ...
    private static bool Overlap((ulong Lower, ulong Upper) first, (ulong Lower, ulong Upper) second) =>
        InRange(first.Lower, second) 
        || InRange(first.Upper, second)
        || InRange(second.Lower, first)
        || InRange(second.Upper, first);

    private static (ulong Lower, ulong Upper) GetOverlap(HashSet<(ulong Lower, ulong Upper)> overlappingRanges)
    {
    ...
    private static bool InRange(ulong number, (ulong Lower, ulong Upper) second) =>
